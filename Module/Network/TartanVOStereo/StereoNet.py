...
class StereoNet7(nn.Module):
    def __init__(self, version=0, uncertainty=False, act_fun='relu'):
        ...
        self.feature_extraction = feature_extraction(last_planes=64,
                                                     bigger=True,
        ...
        self.conv_c2 = Hourglass(2, 64,
                                 0)  # 1/4 #nn.Conv2d(128,128, kernel_size=3, padding=1)
        self.conv_c2_SSP = SSP(64)  # 1/4
        self.conv_c3 = Hourglass(2, 128,
                                 64)  # 1/8 #nn.Conv2d(128,256, kernel_size=3, padding=1)
        self.conv_c4 = Hourglass(2, 192,
                                 64)  # 1/16 #nn.Conv2d(256,256, kernel_size=3, padding=1)
        self.conv_c5 = nn.Conv2d(256, 384, kernel_size=3, padding=1)  # 1/32
        self.conv_c6 = nn.Conv2d(384, 512, kernel_size=3, padding=1)  # 1/64
        self.deconv_c7 = nn.ConvTranspose2d(896, 320, kernel_size=4, stride=2,
                                            padding=1)  # 1/16
        self.deconv_c8 = nn.ConvTranspose2d(576, 192, kernel_size=4, stride=2,
                                            padding=1)  # 1/8
        self.conv_c8 = Hourglass(2, 192, 0)  # 1/8
        self.deconv_c9 = nn.ConvTranspose2d(384, 128, kernel_size=4, stride=2,
                                            padding=1)  # 1/4
        self.conv_c9 = Hourglass(2, 128, 0)  # 1/4
        self.deconv_c10 = nn.ConvTranspose2d(256, 64, kernel_size=4, stride=2,
                                             padding=1)  # 1/2
        self.conv_c10 = Hourglass(2, 64, 0)  # 1/2
        self.deconv_c11 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2,
                                             padding=1)  # 1/1
        ...
        self.deconv_c7_2 = nn.ConvTranspose2d(512, 512, kernel_size=4, stride=2,
                                              padding=1)  # 1/32
